//http://www.htmlgoodies.com/html5/tutorials/introducing-html-5-web-workers-bringing-multi-threading-to-javascript.html#fbid=iKeTS-gpsT0
function CalculatePi(loop)
{
    ...
    if (isNaN(c) || f != c ) {
	  ...
	  var step=Math.floor(c/100);
    for (var i=0,Pi=0;i<=c;i++) {
      Pi=Pi+(4/n)-(4/(n+2));
      n=n+4;
      if (Math.floor(i/step)==(i/step)) self.postMessage({'PiValue': Pi});
    }
